...
- **校准**：读数可带设备；仅 `result=pass` 且在有效期内的校准算有效。设备必须**已关联该监测点、指标匹配且未报废**，否则写入前拒绝（`device_not_linked_to_point` / `device_metric_mismatch` / `device_retired`）。无有效校准（过期/缺失/最近校准不合格）时读数仍入库，但判定 `cal_expired` 异常。
